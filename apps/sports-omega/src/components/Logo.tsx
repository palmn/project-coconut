...
export function Logo({ size = "md", showText = true, animate = false, variant = "default" }: LogoProps) {
  const sizes = {
  ...
  return (
    ...
    >
      <div className="relative" style={{ width: iconSize, height: iconSize }}>
        ...
        >
          {/* Background circle - metallic gold */}
          <defs>
          ...
          <circle cx="61.44" cy="61.44" r="61.44" fill="url(#goldGradient)" />
      ...
      {showText && (
        <div className="flex flex-col leading-none">
          <div className={`font-bold ${textSize}`}>
            {variant === "light" ? (
              <>
                ...
                <span className="text-[#d4af37] drop-shadow-[0_0_8px_rgba(212,175,55,0.5)]">Odds</span>
              </>
            ) : (
              <>
                <span className="text-white">VM</span>
          ...
          <div className={`text-xs uppercase tracking-wider mt-0.5 ${variant === "light" ? "text-white/80" : "text-white/60"}`}>
            Jämför & Vinn
          </div>
        </div>
      )}
